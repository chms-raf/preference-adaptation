    ...
    for iteration = 1:1:maxIterations
        ...
        if e <= epsilon
            ...
            if length(unique(qValue(state_idx,:))) == 1
                % All Q-values are the same, pick random 2
                idx = randperm(length(actions),2);
                action_arr = actions(idx);
            else
...
%             end
        ...
        if action_arr(1) == "input"
            ...
            % If the predicted action is the same as the user's desire,
        ...
        if isInput
            % NOTE: This sample target is NOT the optimal value. This is
            % key. We are mixing on-policy and off-policy Q-leanrning here.
