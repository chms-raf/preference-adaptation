...
% This version also tests out using a pseudo qValue that doesn't decay for
% the optimal policy
...
epsilon = 0.2;     % If epsilon is 1, we only explore, if epsilon is 0, we only exploit
simulate = 1;
...
for episode = 1:1:numEpisodes
    ...
    for iteration = 1:1:maxIterations
        ...
        if pseudo_qValue(state_idx, 1) <= input_threshold        
            ...
            inCount = inCount + 1;
            if simulate == 1
                action = simQuery_adversary(initialState, food_num, food_per_plate);
            else
            ...
            pseudo_qValue(state_idx, 1) = (1-alpha_i)*qValue(state_idx, 1) + alpha_i*r_input;
        else
        ...
        pseudo_qValue(state_idx,1) = (1 - decay)*pseudo_qValue(state_idx,1);
...
%         fprintf('%s --> %s --> %s\n\n', initialState, action, finalState);
        ...
        pseudo_qValue(finalState_idx, action_idx) = (1-alpha)*qValue(finalState_idx, action_idx) + alpha*sample;
        state = finalState;
...
%         if food_num == 0
%             warning('No more food. Quitting episode %i\n', episode)
%             break
%         end
    end
    ...
    pseudo_qValue_array(:,:,episode) = pseudo_qValue;
end
...
% Number of Inputs required per plate
figure
...
figure
subplot(8,3,1)
plot(squeeze(pseudo_qValue_array(1,1,:)))
title('Eat/Input')
ylabel('Pseudo Q-Value')

subplot(8,3,2)
plot(squeeze(pseudo_qValue_array(1,2,:)))
title('Eat/Stay')

subplot(8,3,3)
plot(squeeze(pseudo_qValue_array(1,3,:)))
title('Eat/Quit')

subplot(8,3,4)
plot(squeeze(pseudo_qValue_array(2,1,:)))
title('Anomaly/Input')
ylabel('Pseudo Q-Value')

subplot(8,3,5)
plot(squeeze(pseudo_qValue_array(2,2,:)))
title('Anomaly/Stay')

subplot(8,3,6)
plot(squeeze(pseudo_qValue_array(2,3,:)))
title('Anomaly/Quit')

subplot(8,3,7)
plot(squeeze(pseudo_qValue_array(3,1,:)))
title('Success/Input')
ylabel('Pseudo Q-Value')

subplot(8,3,8)
plot(squeeze(pseudo_qValue_array(3,2,:)))
title('Success/Stay')

subplot(8,3,9)
plot(squeeze(pseudo_qValue_array(3,3,:)))
title('Success/Quit')

subplot(8,3,10)
plot(squeeze(pseudo_qValue_array(4,1,:)))
title('Fail/Input')
ylabel('Pseudo Q-Value')

subplot(8,3,11)
plot(squeeze(pseudo_qValue_array(4,2,:)))
title('Fail/Stay')

subplot(8,3,12)
plot(squeeze(pseudo_qValue_array(4,3,:)))
title('Fail/Quit')

subplot(8,3,13)
plot(squeeze(pseudo_qValue_array(5,1,:)))
title('E-Stop/Input')
ylabel('Pseudo Q-Value')

subplot(8,3,14)
plot(squeeze(pseudo_qValue_array(5,2,:)))
title('E-Stop/Stay')

subplot(8,3,15)
plot(squeeze(pseudo_qValue_array(5,3,:)))
title('E-Stop/Quit')

subplot(8,3,16)
plot(squeeze(pseudo_qValue_array(6,1,:)))
title('Food/Input')
ylabel('Pseudo Q-Value')

subplot(8,3,17)
plot(squeeze(pseudo_qValue_array(6,2,:)))
title('Food/Stay')


subplot(8,3,18)
plot(squeeze(pseudo_qValue_array(6,3,:)))
title('Food/Quit')

subplot(8,3,19)
plot(squeeze(pseudo_qValue_array(7,1,:)))
title('No Food/Input')
ylabel('Pseudo Q-Value')

subplot(8,3,20)
plot(squeeze(pseudo_qValue_array(7,2,:)))
title('No Food/Stay')

subplot(8,3,21)
plot(squeeze(pseudo_qValue_array(7,3,:)))
title('No Food/Quit')

subplot(8,3,22)
plot(squeeze(pseudo_qValue_array(8,1,:)))
title('End/Input')
ylabel('Pseudo Q-Value')
xlabel('Plate Number')

subplot(8,3,23)
plot(squeeze(pseudo_qValue_array(8,2,:)))
title('End/Stay')
xlabel('Plate Number')

subplot(8,3,24)
plot(squeeze(pseudo_qValue_array(8,3,:)))
title('End/Quit')
xlabel('Plate Number')
